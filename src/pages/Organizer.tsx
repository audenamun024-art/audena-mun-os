...
import { Calendar, Users, DollarSign, Plus, Check, X, Shield, UserPlus, UserMinus } from "lucide-react";
...
const Organizer = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      if (evts && evts.length > 0) {
        ...
        const { data: regs } = await supabase.from("registrations").select("*").in("event_id", eventIds).order("created_at", { ascending: false }).limit(20);
        setApplications(regs || []);
        setStats(prev => ({ ...prev, delegates: regs?.length || 0 }));

        // Fetch EB access
        const { data: eb } = await supabase.from("eb_access").select("*, profiles!eb_access_user_id_fkey(full_name, institution)").in("event_id", eventIds).eq("active", true);
        setEbMembers(eb || []);
      }
    };
    fetch();
  ...
  const handleGrantEB = async () => {
    if (!ebEmail || !ebEventId) { toast.error("Select event and enter email"); return; }
    // Find user by email via profiles (simplified - in prod you'd use an edge function)
    ...
    toast.success("EB access granted for this event");
    setEbEmail("");
  };

  const handleRevokeEB = async (id: string, userId: string) => {
  ...
  const tabs = [
    ...
    { key: "applications", label: `Applications (${applications.filter(a => a.status === "pending").length})` },
    { key: "eb", label: "EB System" },
  ] as const;
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border px-4 h-12 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] bg-accent/20 text-accent px-2 py-0.5 rounded-full font-medium">Organizer</span>
        </div>
              ...
              <div key={app.id} className="flex items-center justify-between p-4 bg-card rounded-xl border border-border">
                ...
                {app.status === "pending" ? (
                  <div className="flex gap-1">
                    <Button size="sm" className="bg-accent hover:opacity-90 text-accent-foreground h-7 w-7 p-0" onClick={() => handleApproveReg(app.id)}>
          ...
          <section className="space-y-4 animate-fade-in">
            <div className="bg-card rounded-xl border border-border p-4 space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                EB members get temporary access to Crisis Mode, Admin tools, and Excel exports for the duration of the event.
              </p>
            ...
            <div>
              ...
              {ebMembers.length === 0 ? (
                ...
                <div key={eb.id} className="flex items-center justify-between p-3 bg-card rounded-xl border border-border mb-2">
                  <div>
                    <p className="text-sm font-medium text-foreground">{(eb.profiles as any)?.full_name || "Unknown"}</p>
                  ...
                  <Button
                    ...
                    onClick={() => handleRevokeEB(eb.id, eb.user_id)}
                  >
